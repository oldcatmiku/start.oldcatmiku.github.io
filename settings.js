...
function moreOther(){
	document.getElementById("moreThsOut").innerHTML=`
	<br><p style="text-align:left;word-break:break-all">
	• 已经把访问地址修改成start.oldcatmiku.top，现在可以更加及时地更新项目，背景图片放进了图床。挖坑：自定义背景图和黑暗模式。
	<br>
	• 根据欧盟ePrivacy规定，也称为欧洲 Cookie 法，强制要求在进行任何写入或读取 cookie 和其他跟踪器的操作之前获得用户的同意，但有一些例外。例如本网页用于自定义用户界面的 Cookie 不受到限制，所以没关系。
	<br>
	• 老猫的另一个网站：<a style="color:blue;text-decoration:none;" href="https://txc233.cn" target="_blank" >TXC团队主页</a>
	<br>
	• 有个奇怪的东西：<a style="color:red" onclick="doNotTouch()">别点我</a></p>
	`
}
function doNotTouch(){
